...
const history = (state = initialState, action: any) => {
  switch (action.type) {
    ...
    case 'ADD_HISTORY':
      ...
      tmpH[action.data.id] = { ...action.data, chap: action.chap, time: moment().valueOf() }
      return Object.assign({}, state, {
        history: tmpH,
      });
    ...
    case 'ADD_DOWNLOAD':
      ...
      if (!tmpD[action.data.id]) {
        tmpD[action.data.id] = {
          ...action.data, chaps: "{}", time: moment().valueOf()
        }
      } else {
        tmpD[action.data.id] = {
          ...action.data, chaps: tmpD[action.data.id].chaps, time: moment().valueOf()
        }
      }
