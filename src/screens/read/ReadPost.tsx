...
const ReadPost = (props: any) => {
    ...
    const chaps = useSelector((state: any) => state.chapReducer.chaps[dataBook.id])
    const chapDowload = useSelector((state: any) => state.history.dowload[dataBook.id])?.chaps
    let chapJson: any = {}
    if (chapDowload) {
    ...
    useEffect(() => {
        ads.showFullBanner(() => {
            SimpleToast.show('Cảm ơn bạn đã theo dõi quảng cáo nha !')
        })
    ...
    for (let i: any = 0; i < images.length; i++) {
        sourceImage = sourceImage + `<img src="${chapJson[chapSelect.id] ? "data:image/jpeg;base64," + images[i] : (images[i] + (images[i]?.includes('mobile=2') ? "" : "&mobile=2"))}" style="width:100%"/>`
    }
    let dataHtml = `<html>
		<body>
  ...
  window.ReactNativeWebView.postMessage(JSON.stringify(window.location));
    ...
    const renderActionBar = () => <>
        <View style={styles.a0536abe033bc11ec92980fa8d3b47c02}>
            ...
            <TText style={styles.a0536abe133bc11ec92980fa8d3b47c02}>{chapSelect.name}</TText>
        ...
        <View style={styles.a0536d2f133bc11ec92980fa8d3b47c02}>
            <TouchableOpacity
                ...
                onPress={() => {
                    if (ref.current == length - 1) return SimpleToast.show("Không còn chương nào trước đó", SimpleToast.SHORT)
            ...
            <TouchableOpacity
                ...
                onPress={() => {
                    if (ref.current == 0) return SimpleToast.show("Bạn đang ở chương cuối", SimpleToast.SHORT)
                    ref.current = ref.current - 1
                    setChapselect(chaps[ref.current])
                }}>
    ...
    return <View style={styles.container}>
        {showLoading ? <View style={{
            ...
            }}>Chờ xíu nha! chúng mình đang tải thông tin truyện...</TText>
        </View> : null}
        <WebView
            decelerationRate={24000}
            onScroll={onScroll}
            javaScriptEnabled={true} onMessage={onMessage} style={styles.a0536fa0033bc11ec92980fa8d3b47c02} source={{
                html: dataHtml
            }} />
        ...
        <Modal transparent visible={showChaps} animationType='slide'>
            <View style={styles.a0536fa0133bc11ec92980fa8d3b47c02}>
                <View style={styles.a0537211033bc11ec92980fa8d3b47c02}>
                    <TText style={styles.a0537211133bc11ec92980fa8d3b47c02}>Danh sách chương</TText>
                    <TouchableOpacity onPress={toogleShowChaps}>
                        <Icon size={30} color='#222222' name='close' />
                    </TouchableOpacity>
                </View>
                <Chaps data={dataBook} onScroll={() => { }} headerHeight={-50} onRead />
            </View>
        </Modal>
    </View>;
};
